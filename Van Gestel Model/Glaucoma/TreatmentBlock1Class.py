...
class TreatmentBlock1(object):
    ...
    def update (self):
        ...
        if self.params['IOPReduction'] < 0.1 and self.medicalRecords['MedicationIntake'] > 0 and self.medicalRecords['ContinueTreatment'] == True:
            self.medicalRecords['TreatmentOverallStatus'] =1
            self.DoctorPrescription()
            
        if self.medicalRecords['TreatmentOverallStatus'] == 2 and self.medicalRecords['MedicationIntake'] > 0:
            self.medicalRecords['ContinueTreatment'] = True
            self.DoctorPrescription()
    ...
    def DoctorPrescription(self):
        ...
        if self.medicalRecords['MedicationPath'][0] == 0:
            ...
            if  self.medicalRecords['CurrentMedicationType'] == 10 and self.params['FirstProgression'] == 1:
                self.medicalRecords['TreatmentBlock'] = self.medicalRecords['TreatmentBlock'] +1
