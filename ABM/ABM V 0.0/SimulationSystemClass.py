...
#            obj.CostAttribute['TotalCost'] += (obj.medicalRecords['NumberVF'] *150)
    ...
    def SystemSimulation (self):
        ...
        for i in range(self.size):
            #self.monitor,
            self.patientlist.append( Patient(env,i,{'IOP':self.list_IOP[i],'MD': self.list_MD[i],'MDR':self.list_MDR[i],'CumulativeMDR': 0,'IOPTarget': 24,'Age':self.list_Age[i], 'TrabeculectomyIOP': 0}))
